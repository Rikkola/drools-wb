    ...
    public boolean subsumes( Object other ) {
        if ( other instanceof NumericIntegerConditionInspector ) {
            ...
            if ( anotherPoint != null ) {
                if ( (anotherPoint.getOperator().equals( Operator.LESS_THAN ) && operator.equals( Operator.LESS_THAN_OR_EQUALS )) ) {
                    return covers( anotherPoint.getValue() - 1 );
                } else if ( (anotherPoint.getOperator().equals( Operator.GREATER_THAN_OR_EQUALS ) && operator.equals( Operator.GREATER_THAN )) ) {
                    if ( getValue().equals( anotherPoint.getValue() - 1 ) ) {
                        return true;
                    }
                } else if ( (anotherPoint.getOperator().equals( Operator.GREATER_THAN ) && operator.equals( Operator.GREATER_THAN_OR_EQUALS )) ) {
                    return covers( anotherPoint.getValue() + 1 );
                } else if ( (anotherPoint.getOperator().equals( Operator.LESS_THAN_OR_EQUALS ) && operator.equals( Operator.LESS_THAN )) ) {
                    if ( getValue().equals( anotherPoint.getValue() + 1 ) ) {
                        return true;
                    }
                }
